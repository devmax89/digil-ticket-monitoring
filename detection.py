...
class AlertGenerator:
    ...
    def _rule_no_data_l3(self, s, d):
        """NO_DATA_L3: confronta data Onesait con data MongoDB/availability.
        
        Logica: se la data Onesait è più recente della data dell'ultimo
        availability di almeno 2 giorni (1gg è fisiologico), i dati si fermano
        sulla piattaforma centrale e non arrivano al DB.
        
        Usa data_mongo se presente, altrimenti last_avail_date come proxy.
        Per availability: il dato è delle 24h precedenti, quindi delta 1 = normale.
        """
        if not d.data_onesait: return
        # Ignora date fittizie (Excel default 1900-01-01)
        if d.data_onesait.year < 2020: return
        # Cerca la data di confronto: data_mongo diretta o last_avail_date
        data_confronto = d.data_mongo
        if data_confronto and data_confronto.year < 2020:
            data_confronto = None
        if data_confronto is None:
            data_confronto = d.last_avail_date
        if data_confronto is None:
            # Nessuna data di confronto e Onesait presente + check_mongo KO → segnala
            if d.check_mongo == "KO":
                if _has_active_ticket_or_jira(d, s): return
                self._add(s, d, "NO_DATA_L3", "HIGH",
                          f"Onesait attivo ({d.data_onesait}) ma MongoDB KO — dati bloccati in piattaforma",
                          {"onesait": str(d.data_onesait), "mongo": "KO"})
            return
        if d.data_onesait <= data_confronto:
            return  # Tutto ok
        delta = (d.data_onesait - data_confronto).days
        if delta <= 1: return  # Delta 1gg è fisiologico
        if _has_active_ticket_or_jira(d, s): return
